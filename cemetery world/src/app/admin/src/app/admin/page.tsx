...
export default async function AdminPage() {
  const [{ count: graves }, { count: cemeteries }, { count: countries }, { count: cities }, { count: requests }] = await Promise.all([
  ...
  const actions = [
    { href: '/admin/add-country', icon: '🌍', title: 'הוסף מדינה', desc: 'הוספת מדינה חדשה למאגר' },
    { href: '/admin/add-city', icon: '🏙', title: 'הוסף עיר', desc: 'הוספת עיר בתוך מדינה קיימת' },
    { href: '/admin/add-cemetery', icon: '⚰', title: 'הוסף בית עלמין', desc: 'הוספת בית עלמין בתוך עיר קיימת' },
    { href: '/admin/add-grave', icon: '🪦', title: 'הוסף מצבה', desc: 'הוספה ידנית עם טופס מלא' },
    ...
    <main style={{ padding: '2.5rem 0 4rem' }}>
      <div className="container-site">
        <div dir="rtl">
          ...
          {/* Stats */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: '2.5rem' }}>
            {stats.map(stat => (
          ...
          {/* Flow hint */}
          <div style={{ background: 'linear-gradient(135deg, #f9f6f0, #f0ece3)', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '2rem', border: '1px solid var(--stone-dark)' }}>
            <p style={{ fontWeight: 600, fontSize: '0.9rem', marginBottom: 6 }}>📋 סדר ההוספה הנכון</p>
            <p style={{ fontSize: '0.82rem', color: 'var(--ink-muted)', lineHeight: 1.7 }}>
              קודם הוסף <strong>מדינה</strong> ← אחר כך <strong>עיר</strong> ← אחר כך <strong>בית עלמין</strong> ← ואז <strong>מצבות</strong>
            </p>
          </div>

          {/* Actions */}
          <h2 style={{ fontFamily: "'Crimson Pro', serif", fontSize: '1.4rem', marginBottom: '1rem' }}>פעולות</h2>
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {recentRequests.map((req: any) => {
                  ...
                  return (
                    <div key={req.id} style={{ background: 'white', borderRadius: 10, border: '1px solid var(--stone-dark)', padding: '0.9rem 1.1rem', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
